...
//! `notify_one` signals that group. Off the flash path the SAME `notify_one`
//! permit semantics run over a real waker list under an inner mutex: a waiter
//! stores its [`Waker`] under the lock after checking the permit, and a
//! `notify_one` either wakes one stored waker or stores a permit. A `notify_one`
//! with no waiter stores a permit so the next `notified()` resolves immediately.
//! Under flash the wait collapses to zero wall-clock and participates in
//! quiescence; off it, it is a true reactor-free wake. The permit/waiter state is
