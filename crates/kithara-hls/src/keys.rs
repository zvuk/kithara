...
mod tests {
    ...
    async fn key_processor_applied() -> HlsResult<()> {
        ...
        let processor = Box::new(|key: Bytes, _context: KeyContext| {
    ...
    #[tokio::test]
    async fn key_query_params_applied() -> HlsResult<()> {
        let server = TestServer::new().await;
        let (cache, net) = create_test_cache_and_net();

        let mut params = HashMap::new();
        params.insert("auth".to_string(), "token123".to_string());

        let key_manager = KeyManager::new(cache, net, None, Some(params), None);
        let key_url = server.url("/key.bin")?;

        // Note: This test assumes the server tracks query parameters
        let _key = key_manager.get_key(&key_url, None).await;
