...
use kithara_hls::{
    ...
    fetch::FetchManager,
    pipeline::{BaseStream, PipelineEvent, SegmentPayload, SegmentStream},
...
#[path = "fixture.rs"]
mod fixture;
use fixture::{TestServer, create_test_cache_and_net, test_init_data, test_segment_data};
...
async fn build_basestream(master_url: Url, initial_variant: usize) -> BaseStream {
...
async fn basestream_iterates_with_init_and_segments() {
    ...
    let stream = build_basestream(master_url, 0).await;
...
#[timeout(Duration::from_secs(5))]
#[tokio::test]
async fn basestream_seek_restarts_from_index() {
    let server = TestServer::new().await;
    let master_url = server.url("/master.m3u8").expect("url");

    let mut stream = build_basestream(master_url, 0).await;
...
#[timeout(Duration::from_secs(5))]
#[tokio::test]
async fn basestream_stops_after_cancellation() {
    let server = TestServer::new().await;
    let master_url = server.url("/master.m3u8").expect("url");

    let (assets_wrap, net) = create_test_cache_and_net();
    let (fetch, playlist) = make_fetch_and_playlist(assets_wrap.assets().clone(), net);
    let abr = make_abr(0);
    let cancel = CancellationToken::new();
...
async fn basestream_reconnects_and_resumes_same_segment() {
    ...
    let (assets_wrap, net) = create_test_cache_and_net();
    let (fetch_mgr, playlist_mgr) = make_fetch_and_playlist(assets_wrap.assets().clone(), net);
