...
pub(crate) fn run(
    ...
    let frame_prefix = project
        .perf
        .frame_prefix
        .as_deref()
        .unwrap_or(&project.project.name);
    ...
    for run in &isolated {
        ...
        match summarize(&json, 20, frame_prefix) {
            Ok(summary) => {
...
mod tests {
    ...
    fn renders_cards_and_ranking() {
        ...
        let slow = SlowReport {
            ...
            threshold_ms: 1000.0,
            primary_lane: "flash-on-http".to_owned(),
            tests: vec![SlowTest {
                suite: "s".to_owned(),
