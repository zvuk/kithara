...
use kithara_platform::{Mutex, tokio::sync::mpsc};
...
type BoxStream = Pin<Box<dyn futures::Stream<Item = FetchCmd> + Send>>;
...
struct Inner {
    ...
    chunk_timeout: kithara_platform::time::Duration,
...
impl Downloader {
    ...
    pub fn register<S>(&self, stream: S) -> TrackHandle<S>
    where
    ...
    fn ensure_spawned(&self) {
        ...
        if let Some(handle) = &self.inner.runtime {
            ...
            kithara_platform::tokio::task::spawn(async move { this.run(rx).await });
        }
    }
    ...
    async fn execute_one(
        ...
        loop {
            let chunk = kithara_platform::tokio::select! {
                c = byte_stream.next() => c,
                () = kithara_platform::time::sleep(chunk_timeout) => None,
            };
            match chunk {
                Some(Ok(data)) => {
                    if let Err(io_err) = (cmd.writer)(data.as_ref()) {
                        debug!(?url, bytes_written, "writer error: {io_err}");
                        (cmd.on_complete)(FetchResult::Err(kithara_net::NetError::Http(
                            io_err.to_string(),
                        )));
                        return;
                    }
                    ...
                    if let Some(ref throttle) = cmd.throttle {
                        while throttle() {
                            kithara_platform::time::sleep(
                                kithara_platform::time::Duration::from_millis(10),
                            )
                            .await;
                        }
                    }
