...
use kithara_worker::{AsyncWorker, Fetch, Worker};
    ...
    S: Source<Item = u8>,
{
    ...
    data_rx: kanal::Receiver<Fetch<ByteChunk>>,
    ...
    S: Source<Item = u8>,
{
    ...
    pub fn new(source: Arc<S>, params: SyncReaderParams) -> Self {
        ...
        let worker = AsyncWorker::new(prefetch_source, cmd_rx.to_async(), data_tx.to_async());
    ...
    fn process_item(&mut self, item: Fetch<ByteChunk>, buf: &mut [u8]) -> Option<usize> {
