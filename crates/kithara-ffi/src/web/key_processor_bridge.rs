...
pub(crate) fn install_main_processor(processor: Arc<dyn FfiKeyProcessor>) {
    *BRIDGE.processor.lock_sync() = Some(processor);
    let mut tx_guard = BRIDGE.request_tx.lock_sync();
    if tx_guard.is_none() {
        let (tx, rx) = mpsc::channel();
        *tx_guard = Some(tx);
        *BRIDGE.request_rx.lock_sync() = Some(rx);
    }
}
...
/// pending.
pub(crate) fn pump() {
    let rx_guard = BRIDGE.request_rx.lock_sync();
    ...
    while let Ok(msg) = rx.try_recv() {
        let bytes = BRIDGE
            ...
            .unwrap_or_default();
        let _ = msg.reply.send_sync(bytes);
    }
}
