...
impl BridgeWriter {
    pub fn push(&self, bytes: Bytes) -> IoResult<()> {
        ...
        let current = self.current_buffer_bytes.load(std::sync::atomic::Ordering::Acquire);
        if current + bytes_len > self.max_buffer_bytes {
            return Err(IoError::BufferFull);
        }
        
        // Reserve space
        self.current_buffer_bytes.fetch_add(bytes_len, std::sync::atomic::Ordering::AcqRel);
        ...
        self.tx.send(msg).map_err(|_| {
            ...
            self.current_buffer_bytes.fetch_sub(bytes_len, std::sync::atomic::Ordering::AcqRel);
            IoError::ChannelClosed
        })
    }

    pub fn finish(&self) -> IoResult<()> {
        self.tx.send(BridgeMsg::EndOfStream).map_err(|_| IoError::ChannelClosed)
    }
}
...
impl BridgeReader {
    fn new(rx: Receiver<BridgeMsg>, buffer_bytes_used: std::sync::Arc<std::sync::atomic::AtomicUsize>) -> Self {
        Self {
            rx,
            current_data: None,
            read_pos: 0,
        ...
        if let Some(ref data) = self.current_data {
            ...
            if remaining > 0 {
                ...
                if self.read_pos == data.len() {
                    ...
                    self.buffer_bytes_used.fetch_sub(self.current_data_size, std::sync::atomic::Ordering::AcqRel);
...
mod tests {
    ...
    fn test_read_returns_0_only_after_finish() {
        ...
        writer.push(Bytes::from("test")).expect("push should succeed");
    ...
    fn test_multiple_small_reads() {
        ...
        loop {
            let bytes = reader.read(&mut remaining_buf).expect("read should succeed");
            if bytes == 0 {
                break;
            }
            total_read += bytes;
    ...
    fn test_backpressure_blocks_push_when_full() {
        ...
        assert!(writer.push(Bytes::from("10 bytes__")).is_ok(), "first push should succeed");
        assert!(writer.push(Bytes::from("10 bytes__")).is_ok(), "second push should succeed");
        ...
        assert!(matches!(result, Err(IoError::BufferFull)), "should return BufferFull error");
    }
