...
pub(crate) fn decoder_resampler_target_rate(
...
mod tests {
    ...
    fn current_target_rate(rate: Option<&Arc<AtomicU32>>) -> Option<u32> {
    ...
    fn apple_backend_uses_fused_output_rate_and_omits_stage() {
        ...
        let target = decoder_resampler_target_rate(DecoderBackend::Apple, &host_rate);

        assert_eq!(current_target_rate(target.as_ref()), Some(48_000));
        assert_eq!(
            resampler_stage(
    ...
    fn non_apple_backend_keeps_resampler_stage() {
        ...
        let target = decoder_resampler_target_rate(DecoderBackend::Symphonia, &host_rate);
    ...
    fn non_fused_build_keeps_decoder_source_rate_and_resampler_stage() {
        ...
        assert!(decoder_resampler_target_rate(DecoderBackend::default(), &host_rate).is_none());
        assert_eq!(
            resampler_stage(
