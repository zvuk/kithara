...
pub(crate) struct PlayerCore {
    ...
    pub(crate) playback_rate_shared: Arc<AtomicF32>,
...
mod tests {
    ...
    fn set_rate_updates_shared_atomic() {
        let player = PlayerImpl::new(PlayerConfig::default());
        player.set_rate(2.0);
        let shared = &player.core.playback_rate_shared;
    ...
    fn set_rate_clamps_invalid_values() {
        ...
        assert!(player.core.playback_rate_shared.load(Ordering::Relaxed) >= 0.01);
    ...
    #[kithara::test]
    fn playback_rate_shared_is_address_stable_across_play_pause() {
        let player = PlayerImpl::new(PlayerConfig::default());
        let ptr_before = Arc::as_ptr(&player.core.playback_rate_shared);
        player.play();
        player.pause();
        player.play();
        let ptr_after = Arc::as_ptr(&player.core.playback_rate_shared);
        assert_eq!(
            ptr_before, ptr_after,
            "playback_rate_shared must stay address-stable across transitions"
        );
        player.worker().shutdown();
    }
