...
pub(crate) const fn subscription_config(playing: bool) -> SubscriptionConfig {
    SubscriptionConfig {
        tick_interval_ms: if playing {
            consts::TICK_INTERVAL_ACTIVE_MS
        } else {
            consts::TICK_INTERVAL_IDLE_MS
        },
...
mod tests {
    ...
    #[kithara::test]
    #[case::paused(false, consts::TICK_INTERVAL_IDLE_MS)]
    #[case::playing(true, consts::TICK_INTERVAL_ACTIVE_MS)]
    ...
    fn idle_tick_is_slower_than_active_tick() {
        assert!(
            consts::TICK_INTERVAL_IDLE_MS > consts::TICK_INTERVAL_ACTIVE_MS,
            "idle tick must be slower than active tick to save CPU"
        );
        assert!(
            consts::TICK_INTERVAL_IDLE_MS <= 1000,
