...
async fn seamless_queue_advance_gapless_when_crossfade_is_zero(temp_dir: TestTempDir) {
    ...
    // the same render block where item-1 hits EOF. The two AAC fixtures are
    // separately encoded, so the robust contract is absence of an inserted
    // render-block gap rather than phase-perfect waveform continuity.
...
async fn seamless_queue_advance_overlaps_tracks_when_crossfade_is_non_zero(temp_dir: TestTempDir) {
    ...
    // Both signals must be clearly audible during overlap. The trailing leg
    // uses windowed detection because AAC/fMP4 startup and segment boundaries
    // can smear phase enough for one long Goertzel window to under-report a
    // locally audible tone.
    assert!(
