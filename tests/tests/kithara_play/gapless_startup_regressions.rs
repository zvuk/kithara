...
use kithara_play::{PlayerConfig, Resource, ResourceConfig, impls::offline_backend::OfflineConfig};
use kithara_stream::AudioCodec;
use kithara_test_utils::{
    HlsFixtureBuilder, TestServerHelper, TestTempDir,
    fixture_protocol::{DelayRule, PackagedAudioRequest, PackagedAudioSource, PackagedSignal},
    temp_dir,
};
...
#[case(GaplessMode::SilenceTrim(Default::default()))]
async fn gapless_modes_do_not_block_network_startup_until_full_cache(
    ...
    harness.player().insert(resource, None);
    ...
    loop {
        let (block, _) = harness.render_block_and_drain();
