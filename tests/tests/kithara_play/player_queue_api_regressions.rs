...
use kithara_play::{
    PlayerConfig, PlayerEvent, PlayerImpl, Resource, ResourceConfig,
...
async fn auto_advance_starts_next_track_without_explicit_play(temp_dir: TestTempDir) {
    ...
    while Instant::now() <= deadline {
        let (block, drained) = harness.render_block_and_drain();
        ...
        if first_item_finished.is_none() {
            first_item_finished = events.iter().find_map(|timed| {
                matches!(
                    &timed.event,
                    PlayerEvent::ItemDidPlayToEnd { item_id: Some(id) } if id == first_id.as_ref()
                )
