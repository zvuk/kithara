...
`kithara` — это **networking + decoding** библиотека, но **не полноценный плеер**:
...
- async задачи только доставляют данные/ресурсы и управляют кэшем/оркестрацией,
- между ними — явный “bridge” (канал/буфер) с корректной семантикой EOF и backpressure.
