...
- `kithara-assets`: менеджер **дерева ресурсов на диске**:
  - хранит дерево ресурсов под одним корнем (`root_dir`),
  - предоставляет базовый контракт `Assets` для открытия *сырого* ресурса (`AtomicResource` / `StreamingResource`) по ключу,
  - lease/pin реализуется **декоратором** (`LeaseAssets`) поверх `Assets`,
...
- хранение — это дерево файлов под корнем кэша:
  - путь формируется как `<root_dir>/<asset_root>/<rel_path>`,
...
- открытие ресурсов должно сохранять “ресурсную” модель:
  - маленькие объекты: `AtomicResource` (whole-object `read`/`write` с atomic replace),
  - большие объекты/сегменты: `StreamingResource` (`write_at/read_at` + `wait_range`).
- запись маленьких объектов (playlist, keys, metadata) должна быть crash-safe:
...
Примечание к текущей реализации `kithara-assets` (актуальная реальность):
- базовый контракт `Assets` не реализует lease/pin; lease/pin — это декоратор `LeaseAssets` поверх `Assets`,
- pin table persistится best-effort через `AtomicResource` как статический meta-ресурс в namespace `_index/*`
  (например `_index/pins.json`), FS остаётся source of truth.
...
- распределять тесты по слоям ответственности:
  ...
  - `kithara-assets`:
    ...
    - lease/pin как декоратор поверх `Assets` с best-effort persisted pin table через `AtomicResource` (`_index/pins.json`),
    - best-effort metadata в namespace `_index/*` (можно удалить/потерять; FS остаётся source of truth).
