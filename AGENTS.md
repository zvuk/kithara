...
- Zero-tolerance for lint suppress during the lint-zero-suppress campaign (`.docs/plans/2026-05-24-lint-zero-suppress.md`). Forbidden: new entries in `.config/{arch,style,idioms,ast-grep}/*.toml`; new `#[allow(...)]` / `#[expect(...)]` / `#![allow(...)]` / `// xtask-lint-ignore:` markers in production code. Legalized exceptions: only `// kithara:cancel:owner|bridge`. Unavoidable lint → STOP, escalate to the campaign integrator; never grow `baseline.toml` or add a suppress to make a commit pass. Cast-group fixes use `num-traits` (`NumCast` / `AsPrimitive` / `ToPrimitive`), not `as` or per-site `try_from().expect()`.
