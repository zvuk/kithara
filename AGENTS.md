...
Разрешено:
- добавлять новый код **только** если он:
  1) используется в текущем изменении (есть вызовы/интеграция), **или**
  2) является частью публичного контракта, зафиксированного в `docs/kanban*.md` или README целевого крейта, **и**
  3) имеет тест (или минимальный пример использования), фиксирующий поведение.

Если кажется, что “вот этот helper точно понадобится дальше”:
- **не добавлять**;
- оформить отдельным пунктом в kanban/задачах (или дождаться явного запроса владельца).

Цель: не раздувать поверхность API, не копить мёртвый код и не терять время на последующую чистку/обсуждения.
...
## 0.a) Границы изменений (одно изменение — один крейт)

## 0.b) Форматирование (cargo fmt)
**Правило:** после выполнения каждого чекбокса агент должен:
1) проставить чекбокс выполненным (`[x]`) в соответствующем `docs/kanban-kithara-*.md`;
2) запустить `cargo fmt` для workspace (или минимум для целевого крейта) и убедиться, что форматирование применилось корректно.

Рекомендуемый порядок после каждого чекбокса:
1) отметить выполненный пункт в kanban (`[ ]` → `[x]`) в соответствующем файле `docs/kanban-kithara-*.md`
2) `cargo fmt`
3) `cargo test` (для целевого крейта или нужного набора тестов)
4) если `cargo` в выводе предлагает применить auto-fix (например, `cargo fix ...`) — выполнить **ровно ту команду**, которую предлагает `cargo`, затем повторить `cargo fmt` и перезапустить `cargo test`

> Цель: минимизировать шум в диффах и ловить проблемы форматирования сразу, а не в конце задачи.

## 0.c) Обратная совместимость и legacy-код
**Правило:** мы на этапе активной разработки, поэтому **не сохраняем обратную совместимость** и **не оставляем legacy-код** “на всякий случай”.

- Если требуется изменить API/тип/поведение — меняем напрямую и обновляем использования в рамках **того же крейта**.
- Не добавлять “deprecated” прослойки, алиасы, временные адаптеры и совместимые обёртки, если это не является отдельной явно согласованной задачей.

**Исключение:** если в `kithara/docs/kanban.md` зафиксирован **публичный контракт** для крейта/компонента, то этот контракт считается целью реализации. В этом случае “стабильность” относится к соответствию канбан-контракту, а не к сохранению старых форм/имён/поведения.

**Правило:** работая над задачей в рамках конкретного крейта `crates/<name>`, агент **не должен** изменять код в соседних крейтах.

### Разрешённые исключения (узко и осознанно)
Разрешены только следующие типы изменений вне целевого крейта:
1) **Workspace-first зависимости**: правки корневого `kithara/Cargo.toml` в `[workspace.dependencies]`, если без этого невозможно выполнить задачу в целевом крейте.
2) **Workspace-wide конфиги**: `rustfmt.toml`, `clippy.toml`, `deny.toml` — только если это требуется для исправления блокирующих ошибок сборки/линтов.
3) **Документация планов**: `kithara/docs/kanban.md` и (если нужно) `README.md` целевого крейта.

### Запрещённые примеры (даже если “кажется мелочью”)
- “чуть-чуть поправить” тип/ошибку/публичный API в `kithara-core`, когда задача делается в `kithara-cache`.
- рефакторинг общих утилит в другом крейте “заодно”.

### Если без правок соседнего крейта никак
- Остановиться.
- Зафиксировать минимальное изменение отдельной задачей в канбане (отдельный пункт).
- Дождаться подтверждения/решения владельца репозитория.
...
Важно: правило “одно изменение — один крейт” применяется по умолчанию ко всем задачам, чтобы избегать случайного расползания изменений по workspace.
...
Доп. правило: если владелец репозитория пишет тебе **название задачи** (название должно быть не длинее 50 символов, если длинее, то это не название а тело задачи которое можно взять в работу), ты должен:
1) найти это название в файлах kanban (`docs/kanban-kithara-*.md`), и только потом брать в работу;
2) выполнять задачу в точности по чекбоксам из найденного раздела (без “самодеятельности” и без расширения контракта);
3) по мере выполнения проставлять `[x]` на завершённых чекбоксах.
...
## 0.e) Роли крейтов и запрет scope creep (“не тащить чужую ответственность”)
**Проблема:** агенты иногда “закрывают фичу” тем, что перетаскивают в целевой крейт произвольные операции (например: в networking/source крейт добавляют файловую разметку/системные утилиты), что ломает архитектуру и границы ответственности.

**Правило (нормативно):** работая в `crates/<name>`, агент **не должен** добавлять код, который по смыслу относится к ответственности другого крейта, даже если это кажется самым быстрым способом “сделать чтобы работало”.

Как действовать правильно:
1) Открыть `docs/kanban-kithara-<crate>.md` и прочитать секции “Цель”, “Инварианты”, “Компоненты (контракт и ответственность)”.
2) Любой новый модуль/функция/тип должен явно укладываться в эти границы. Если не укладывается — **не добавлять**.
3) Если без “чужой” ответственности задачу не закрыть:
   - остановиться,
   - оформить это как отдельный пункт в kanban (отдельная задача) и дождаться подтверждения владельца.

Короткая карта ролей (ориентир):
- `kithara-net`: HTTP клиент (timeouts/retries/headers/range), **без** кэшей и без HLS/file логики.
- `kithara-cache`: persistent disk cache + eviction/lease, **без** сетевой логики.
- `kithara-hls`: HLS VOD orchestration (playlists/segments/keys/DRM/ABR), **не** переносить decode или “общие файловые утилиты” сюда.
- `kithara-file`: progressive source orchestration (стрим байтов, range/seek политика), **без** кэш-реализации (только использование `kithara-cache` как фасада).
- `kithara-io`: bridge async bytes → sync `Read+Seek` (EOF/backpressure/seek contract), **без** сети и без HLS/file orchestration.
- `kithara-decode`: Symphonia decode → PCM (worker/pipeline), **без** сети и без HLS/file orchestration.
- `kithara-core`: identity + базовые ошибки, **без** настроек/логики верхних уровней.
