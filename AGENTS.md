...
**Правило:** после выполнения каждого чекбокса агент должен запускать `cargo fmt` для workspace (или минимум для целевого крейта) и убеждаться, что форматирование применилось корректно.

Рекомендуемый порядок после каждого чекбокса:
1) `cargo fmt`
2) `cargo test` (для целевого крейта или нужного набора тестов)
...
- Предпочитать `use`-импорты в начале файла и короткие, читаемые имена в теле.
